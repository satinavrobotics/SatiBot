package org.openbot.mapManagement.rendering;
...
public class BackgroundRenderer {
  ...
  // Flag to track if the renderer has been initialized
  ...
  public void createOnGlThread(Context context, int depthTextureId) throws IOException {
    Timber.d("Creating BackgroundRenderer on GL thread");
    ...
    Timber.d("Generated camera texture with ID: %d", cameraTextureId);
    ...
    {
      Timber.d("Loading camera shaders from: %s and %s", CAMERA_VERTEX_SHADER_NAME, CAMERA_FRAGMENT_SHADER_NAME);
      int vertexShader =
              ShaderUtil.loadGLShader(TAG, context, GLES20.GL_VERTEX_SHADER, CAMERA_VERTEX_SHADER_NAME);
      Timber.d("Loaded vertex shader: %d", vertexShader);

      int fragmentShader =
              ShaderUtil.loadGLShader(
                      TAG, context, GLES20.GL_FRAGMENT_SHADER, CAMERA_FRAGMENT_SHADER_NAME);
      Timber.d("Loaded fragment shader: %d", fragmentShader);
    ...
    // Mark as initialized only after all initialization is complete
    isInitialized = true;
    Timber.d("BackgroundRenderer initialization complete");
  }
  ...
  private void draw(boolean debugShowDepthMap) {
    ...
    Timber.d("Drawing background with texture ID: %d, isInitialized: %b", cameraTextureId, isInitialized);
